...
class ThreadSafeOverlay:
    ...
    def start(self, data_callback: Callable, opacity: float = 0.8) -> bool:
        with self._lock:
    ...
    def set_opacity(self, alpha_value: float) -> bool:
        if not self._running.is_set():
            return False
...
class EDMRNOverlay:
    def __init__(self, data_callback: Callable, initial_opacity: float = 0.8, command_queue: Optional[queue.Queue] = None):
        ...
        self.next_label: Optional[tk.Label] = None
        ...
        self.body_labels: List[tk.Label] = []
    
    def create_window(self):
        try:
            self.root = tk.Tk()
            self.root.title("EDMRN Overlay")
    ...
    def set_opacity(self, alpha_value: float):
        if self.root:
            try:
    ...
    def run(self, running_event: threading.Event):
        try:
            self.create_window()
...
def start_overlay(data_callback: Callable, opacity: float = 0.8) -> Optional[ThreadSafeOverlay]:
    if _overlay_manager.is_running():
        return _overlay_manager
    
    if _overlay_manager.start(data_callback, opacity):
        return _overlay_manager
...
def stop_overlay():
    _overlay_manager.stop()


def get_overlay_data_from_app(app_instance):
    if not app_instance or not hasattr(app_instance, 'route_manager'):
        return None
    
    try:
        route_data = app_instance.route_manager.get_route()
        if not route_data:
            return {
        ...
        visited = [item for item in route_data if item.get('status') == 'visited']
        unvisited = [item for item in route_data if item.get('status') == 'unvisited']
        ...
        return {
            ...
            'total_distance': f"{app_instance.total_distance_ly:.2f} LY",
            'traveled_distance': f"{app_instance.traveled_distance_ly:.2f} LY"
        }
            
    except Exception as e:
        print(f"Overlay data error: {e}")
        return None
