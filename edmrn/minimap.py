...
class MiniMapFrame(ctk.CTkFrame):
    def __init__(self, master, width=480, height=360, on_system_selected=None, **kwargs):
        ...
        self._update_colors()
        self.ed_bg_color = '#0A0A0F'
        self.ed_frame_color = '#1A1A2E'
        self.ed_accent_color = '#FF8C00'
        self.ed_text_color = '#E0E0E0'
        try:
            from matplotlib.figure import Figure
            from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg
            self.fig = Figure(figsize=(6, 4), dpi=100, facecolor=self.ed_bg_color)
            self.ax = self.fig.add_subplot(111, projection='3d', facecolor=self.ed_bg_color)
    ...
    def _setup_axes(self):
        if not self.matplotlib_available or not self.ax:
            return
        try:
            self.ax.set_xlabel('X (LY)', color=self.ed_text_color, fontfamily='Segoe UI')
            self.ax.set_ylabel('Y (LY)', color=self.ed_text_color, fontfamily='Segoe UI')
            self.ax.set_zlabel('Z (LY)', color=self.ed_text_color, fontfamily='Segoe UI')
            self.ax.tick_params(colors=self.ed_text_color)
            self.ax.xaxis.label.set_color(self.ed_text_color)
            self.ax.yaxis.label.set_color(self.ed_text_color)
            self.ax.zaxis.label.set_color(self.ed_text_color)
            ...
            self.ax.grid(color=self.ed_accent_color, linestyle=':', linewidth=0.5, alpha=0.3)
        ...
        try:
            ...
            if not route_list:
                self.ax.text(0.5, 0.5, 0.5, 'No route data',
                           ha='center', va='center',
                           color=self.ed_text_color, fontsize=12, fontfamily='Segoe UI')
                if self.canvas:
                    self.canvas.draw_idle()
                return True
            xs, ys, zs = [], [], []
        ...
        try:
            ...
            legend_elements = [
                Line2D([0], [0], marker='o', color='w',
                      label='Visited', markerfacecolor=COLOR_VISITED, markersize=8, markeredgecolor='#000000'),
                Line2D([0], [0], marker='o', color='w',
                      label='Skipped', markerfacecolor=COLOR_SKIPPED, markersize=8, markeredgecolor='#000000'),
                Line2D([0], [0], marker='o', color='w',
                      label='Pending', markerfacecolor=COLOR_PENDING, markersize=8, markeredgecolor='#000000')
            ]
            self.ax.legend(
                handles=legend_elements,
                loc='lower center',
                ncols=3,
                facecolor=self.ed_frame_color,
                edgecolor=self.ed_accent_color,
                labelcolor=self.ed_text_color,
                fancybox=True,
                shadow=True,
                fontsize='small',
    ...
    def highlight_system(self, name):
        ...
        try:
            ...
            self.ax.set_title(f'Selected: {name}', color=self.ed_accent_color, fontsize=11, fontfamily='Segoe UI', weight='bold')
            if self.canvas:
                self.canvas.draw_idle()
            return True
        except Exception as e:
    ...
    def refresh_colors(self):
        ...
        try:
            self._update_colors()
            if self.fig:
                self.fig.set_facecolor(self.ed_bg_color)
            if self.ax:
                self.ax.set_facecolor(self.ed_bg_color)
            self._setup_axes()
